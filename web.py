"""Flask web application for SecurePass demo.
...
from securepass.storage import load_vault, save_vault, load_vault_with_key, save_vault_with_key
from securepass.crypto import derive_key
from securepass import remote_store


app = Flask(__name__)
app.secret_key = os.environ.get("SECUREPASS_FLASK_SECRET", "dev-secret-key")
# Use server-side filesystem session to avoid putting sensitive data in client cookies
...
if __name__ == '__main__':
    ...
    app.jinja_env.globals.update(current_year=datetime.utcnow().year)
    debug_mode = os.environ.get('FLASK_DEBUG', '0') == '1'
    host = '0.0.0.0' if os.environ.get('FLASK_ENV') == 'production' else '127.0.0.1'
    if app.config['SESSION_COOKIE_SECURE'] and app.secret_key == 'dev-secret-key':
        print('WARNING: Using default secret key in production-like mode; set SECUREPASS_FLASK_SECRET')
